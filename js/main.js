...
document.addEventListener("keydown", (event) => {
  // Escuta as teclas digitadas na página
  if (event.key.length === 1 && ord(event.key) > 64 && ord(event.key) < 91) {
    // Checa se está entre 'A' e 'Z'
    insert(event.key);
  } else if (
    event.key.length === 1 &&
    ord(event.key) > 96 &&
...
function insert(str) {
  ...
  if (str === "∼") {
    ...
    if (is_logical_operator(str) && is_logical_operator(expression.slice(-1)))
      return;
    if (is_logical_operator(str) && expression.slice(-1) === "") return;
  }
  if (
    (is_variable(str) || str === "[Verdadeiro]" || str === "[Falso]") &&
...
function changeMode(mode = "") {
  let [A, B] =
    this.checked || mode === "dark" ? ["light", "dark"] : ["dark", "light"];
